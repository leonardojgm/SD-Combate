  ...
  $('.celulaJogadorA').click(function() { // celula clicada do lado do Jogador A
    if ((etapa_partida === 2) && ($(this).children().length <= 0) && (((vez_jogador == 1) && (Jogador_1.lado_jogador == "jogadorA")) || ((vez_jogador == 2) && (Jogador_2.lado_jogador == "jogadorA"))) && (($(this).css("background-color") == "rgb(255, 255, 0)") || ($(this).css("background-color") == "transparent"))){ // se está na etapa 2 e se cor da celula com mouse em cima ou transparente
      ...
      if (confirm("Jogador " + vez_jogador + ". Confirmar que selecionou a celula " + celula_selecionada + " para colocar " + unidade_selecionada + " ?")) { // mensagem informando a celula selecionada, para confirmação
        ...
        $(this).append('<div id=' + vez_jogador + unidade_selecionada + celula_selecionada + ' class="img_unidade" style=" background-image: url(imagens/' + img_cor_unidade + '.png)"></div>'); // cria um novo div com a imagem da unidade
  ...
  $('.celula').click(function() { // celula clicada no tabuleiro geral
    if ((etapa_partida === 3) && (acao_ataque_defesa == "defesa") && ($(this).children().length <= 0)){ // se está na etapa 3 e se cor da celula com mouse em cima ou transparente de acordo com o ataque e unidade do jogador ou defesa unidade do inimigo
      if (confirm("Jogador " + vez_jogador + ". Confirmar que selecionou a celula " + $(this).attr("id") + " ?")) { // mensagem para confirmar a unidade que clicou
        $("#" + $(this).attr("id")).append('<div id=' + Embates_batalha.unidade_ataque.substring(0,Embates_batalha.unidade_ataque.length - 2) + $(this).attr("id") + ' class="img_unidade" style=" background-image: url(imagens/' + nomeUnidadeImagem(Embates_batalha.unidade_ataque) + '.png)"></div>'); // cria um novo div com a imagem da unidade no lugar da defesa
...
function loginJogadores(lado_jogador){ // etapa onde os jogadores acessam o jogo
  ...
  if ((Jogador_1.id_jogador === 0) || (Jogador_2.id_jogador === 0)){ // verifica se já há alguem logado
    ...
    if ((nome === null)||(nome === "")) {
      ...
      $("#img" + img_lado_jogador).attr("src","imagens/" + cor + "_time.png"); // altera a imagem de acordo com a ordem de logon
      $(nome_lado_jogador).text(nome); // altera o nome do jogador exibido
      $("#ImgBandeira" + img_lado_jogador).attr("src","imagens/" + cor + "_bandeira.png"); // altera a imagem das unidades
      $("#ImgBomba" + img_lado_jogador).attr("src","imagens/" + cor + "_bomba.png");
      $("#ImgEspiao" + img_lado_jogador).attr("src","imagens/" + cor + "_espiao.png");
      $("#ImgSoldado" + img_lado_jogador).attr("src","imagens/" + cor + "_soldado.png");
      $("#ImgCabo_armeiro" + img_lado_jogador).attr("src","imagens/" + cor + "_cabo_armeiro.png");
      $("#ImgSargento" + img_lado_jogador).attr("src","imagens/" + cor + "_sargento.png");
      $("#ImgTenente" + img_lado_jogador).attr("src","imagens/" + cor + "_tenente.png");
      $("#ImgCapitao" + img_lado_jogador).attr("src","imagens/" + cor + "_capitao.png");
      $("#ImgMajor" + img_lado_jogador).attr("src","imagens/" + cor + "_major.png");
      $("#ImgCoronel" + img_lado_jogador).attr("src","imagens/" + cor + "_coronel.png");
      $("#ImgGeneral" + img_lado_jogador).attr("src","imagens/" + cor + "_general.png");
      $("#ImgMarechal" + img_lado_jogador).attr("src","imagens/" + cor + "_marechal.png");
  ...
  $('#tPrinc').find('tr').each(function(indice, value){
    $(this).find('td').each(function(indice, value){
      ...
      if ($(this).children().length <= 0) {
          ...
          $(this).children().append('<div class="img_unidade azul_inimigo" style=" background-image: url(imagens/azul_inimigo.png)"></div>');
      } else if ((etapa_partida === 2) &&(vez_jogador == 2) && ($(this).children().attr('id').substring(0,1) == 2)) {
        $(this).children().append('<div class="img_unidade verde_inimigo" style=" background-image: url(imagens/verde_inimigo.png)"></div>');
      } else if ((etapa_partida === 3) && (vez_jogador == 1) && ($(this).children().attr('id').substring(0,1) != 1)) {
          $(this).children().append('<div class="img_unidade azul_inimigo" style=" background-image: url(imagens/verde_inimigo.png)"></div>');
      } else if ((etapa_partida === 3) && (vez_jogador == 2) && ($(this).children().attr('id').substring(0,1) != 2)) {
        $(this).children().append('<div class="img_unidade verde_inimigo" style=" background-image: url(imagens/azul_inimigo.png)"></div>');
      }
    });
  });
  exibirVezJogador();
